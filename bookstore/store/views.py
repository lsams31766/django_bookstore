...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated():
        ...
        paypalrestsdk.configure({
            'mode': 'sandbox',
            'client_id':'AbPSOfIG88U_zMz1z3GqFuMAchKFIXzFMuuSlLlA2nNZTaBh-ewYHho5qgM-GjWSmUGs9cIXUGB4FWnm',
            'client_secret':'EMTjt24kUL2wBGvV7HeSdXGdpPu71Zo2YzRUgEkYLHxGJsjQhy5yg8P99OC9LFuVakD4VtLiHoNBpDAL'})
        payment = paypalrestsdk.Payment({
            ...
            'redirect_urls': {
                ...
                'cancel_url': 'http://lcoalhost:8000/store'},
            'transactions':[{
                'item_list': {
                    'items': items},
            'amount': {
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = 'sk_test_3lLGBRoKSIV7eRGGXgTda9dr'
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
...
def process_order(request, processor):
    if request.user.is_authenticated():
        if processor == 'paypal':
            ...
            return JsonResponse({'redirect_url':reverse('commplete_order', args=['stripe'])})
    else:
        return redirect('index')
    ...
    if request.user.is_authenticated():
        ...
        if processor == 'paypal':
            ...
            if payment.execute({'payer_id': payment.payer.payer_info.payer_id}):
                ...
                message = "There was a problem with the transaction.  Error %s" % (payment.error.messsage)
            context = {
                'message': message,
            }
            ...
            render(request, 'store/order_complete.html', context)
        else:
            return redirect('index')
